...
# TODO: Create a yaml file for the initial pose and load it here as a parameter
def main(args=None):
    rclpy.init(args=args)
    nav2_simple_commander = BasicNavigator()
    q_x, q_y, q_z, q_w = quaternion_from_euler(0.0, 0.0, 0.0)
    initial_pose = PoseStamped()
    initial_pose.header = Header(frame_id='map', stamp=nav2_simple_commander.get_clock().now().to_msg())
